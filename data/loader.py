...
def load_cam(resolution, id, cam_info, resolution_scale):
...
def load_cameras(cam_infos, resolution_scale, resolution):
    camera_list = []

    for id, c in enumerate(cam_infos):
        camera_list.append(load_cam(resolution, id, c, resolution_scale))

    return camera_list

def load_dataset(source_path, images_folder, eval, white_background=True, resolution_scale=1.0, resolution=-1):
    ...
    train_cameras = load_cameras(scene_info.train_cameras, resolution_scale, resolution)
    print(f"Loaded Train Cameras: {len(train_cameras)}")
    test_cameras = load_cameras(scene_info.test_cameras, resolution_scale, resolution)
